...
public class Formatter {
    ...
    public static String getKilometersPerHour(float metersPerSecond) {
        return String.format(Locale.getDefault(), "%.01f km/h", metersPerSecond * 3600f / 1000);
    }
    ...
    public static String getFormattedDistance(float meters) {
        ...
        if (meters > 999) {
            formattedDistance = String.format(Locale.getDefault(), "%.01f km", Math.ceil(meters) / 1000f);
        } else {
            formattedDistance = String.format(Locale.getDefault(), "%d m", Math.round(meters));
        }
